class NBAStatsBoxscoreParserV3:
    ...
    def get_team_headers(self, headers=tuple(), level=0):
        if level == 0:
            ...
            headers = headers + tuple([header for header in tmp.keys() if header not in ('players', 'statistics')])
            return self.get_team_headers(headers, level=2)
        else:
            try:
                tmp = self.nba_dict[list(self.nba_dict.keys())[1]]['homeTeam']['statistics']
            except KeyError:
                return list(headers)
    ...
    def get_players_headers(self, headers=tuple(), level=0):
        if level == 0:
            ...
            headers = headers + tuple([header for header in tmp.keys() if header != 'statistics'])
            return self.get_players_headers(headers, level=3)
        else:
            tmp = self.nba_dict[list(self.nba_dict.keys())[1]]['homeTeam']['players'][0]['statistics']
            headers = headers + tuple([header for header in tmp.keys()])
            return list(headers)
    ...
    def get_team_data(self):
        ...
        home_team_info = [value for key, value in raw_dict['homeTeam'].items() if key not in ('players', 'statistics')]
        home_team_stats = [x for x in raw_dict['homeTeam']['statistics'].values()]

        away_team_info = [value for key, value in raw_dict['awayTeam'].items() if key not in ('players', 'statistics')]
        away_team_stats = [x for x in raw_dict['awayTeam']['statistics'].values()]
        return [[raw_dict['gameId']] + home_team_info + home_team_stats,
    ...
    def get_player_data(self):
        ...
        for team in ['awayTeam', 'homeTeam']:
            team_info = [[value for key, value in raw_dict[team].items() if key not in ('players', 'statistics')]]
            n_pl = len(raw_dict[team]['players'])
            play_info = [[value for key, value in raw_dict[team]['players'][i].items() if key != 'statistics'] for i in
                         range(len(raw_dict[team]['players']))]
            play_stats = [[value for key, value in raw_dict[team]['players'][i]['statistics'].items()] for i in
                          range(len(raw_dict[team]['players']))]
            stats = [[x, *y, *z, *w] for x, y, z, w in zip(game_id * n_pl, team_info * n_pl, play_info, play_stats)]
            data = data + stats
        return data
...
class NBAStatsBoxscoreTraditionalParserV3(NBAStatsBoxscoreParserV3):
    ...
    def get_start_bench_data(self):
        raw_dict = self.nba_dict[list(self.nba_dict.keys())[1]]
        home_team_info = [value for key, value in raw_dict['homeTeam'].items() if
                          key not in ('players', 'statistics', 'starters', 'bench')]
        home_team_starter_stats = [x for x in raw_dict['homeTeam']['starters'].values()] + ['Starters']
        home_team_bench_stats = [x for x in raw_dict['homeTeam']['bench'].values()] + ['Bench']

        away_team_info = [value for key, value in raw_dict['awayTeam'].items() if
                          key not in ('players', 'statistics', 'starters', 'bench')]
        away_team_starter_stats = [x for x in raw_dict['awayTeam']['starters'].values()] + ['Starters']
        away_team_bench_stats = [x for x in raw_dict['awayTeam']['bench'].values()] + ['Bench']

        return [[raw_dict['gameId']] + home_team_info + home_team_starter_stats,
    ...
    def get_data_sets(self):
        ...
        results['TeamStats'] = {'headers': team_head, 'data': team_data}
        results['PlayerStats'] = {'headers': player_head, 'data': pl_data}
        results['TeamStarterBenchStats'] = {'headers': start_bench_head, 'data': start_bench_data}
        return results
...
class NBAStatsBoxscoreMatchupsParserV3:
    ...
    def get_players_headers(self, headers=tuple(), level=0):
        if level == 0:
            ...
            tmp = self.nba_dict[list(self.nba_dict.keys())[1]]['homeTeam']['players'][0]
            headers = headers + tuple([header + 'Def' for header in tmp.keys() if header != 'matchups'])
            return self.get_players_headers(headers, level=3)
        elif level == 3:
            tmp = self.nba_dict[list(self.nba_dict.keys())[1]]['homeTeam']['players'][0]['matchups'][0]
            headers = headers + tuple([header + 'Off' for header in tmp.keys() if header != 'statistics'])
            return self.get_players_headers(headers, level=4)
        else:
            tmp = self.nba_dict[list(self.nba_dict.keys())[1]]['homeTeam']['players'][0]['matchups'][0]['statistics']
            headers = headers + tuple([header for header in tmp.keys()])
            return list(headers)
        ...
        for team in ['homeTeam', 'awayTeam']:
            ...
            for i, def_pl in enumerate(tmp[team]['players']):
                for j, off_pl in enumerate(tmp[team]['players'][i]['matchups']):
                    def_data = [value for key, value in tmp[team]['players'][i].items() if key != 'matchups']
