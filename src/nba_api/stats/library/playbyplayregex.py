...
pattern_player = rf"(?P<player>{pattern_player_name_anomaly}(\w+{pattern_player_char}{pattern_player_suffix}))"
...
pattern_field_goal_made = (
    rf"^{pattern_player} (((?P<distance>\d+)\' )| )?[ ]*(?P<field_goal_type>[\w+( |\-)]*) "
    r"\((?P<points>\d+) PTS\)( \((?P<player_ast>\D+) (?P<assists>\d+) AST\))?$"
)

pattern_field_goal_missed = rf"^MISS {pattern_player} ((?P<distance>\d+)\' )?[ ]*(?P<field_goal_type>[\w+( |\-)]*)$"
...
pattern_turnover_player = (
    rf"^{pattern_player} (?P<turnover_type>[\w+?\-? ]*) (Turnover[ ]?)+ "
...
re_field_goal_made = re.compile(
    pattern_field_goal_made
)  # Example: "Stephen Curry 26' 3PT Jump Shot (3 PTS)"
re_field_goal_missed = re.compile(
    pattern_field_goal_missed
)  # Example: "MISS Stephen Curry 3PT Jump Shot"
re_free_throw_made = re.compile(
    pattern_free_throw_made
)  # Example: "Stephen Curry Free Throw 1 of 2 (1 PTS)"
re_free_throw_miss = re.compile(
    pattern_free_throw_miss
)  # Example: "MISS Stephen Curry Free Throw 2 of 2"
...
re_rebound_player = re.compile(
    pattern_rebound_player
)  # Example: "Stephen Curry REBOUND (Off:1 Def:3)"
re_rebound_team = re.compile(pattern_rebound_team)  # Example: "Warriors Rebound"

# Violations and fouls
re_foul_player = re.compile(
    pattern_foul_player
)  # Example: "Draymond Green P.FOUL (P1.T3) (J.Smith)"
re_foul_team = re.compile(
    pattern_foul_team
)  # Example: "WARRIORS T.Foul (Defensive three seconds) (M.Davis)"
re_violation = re.compile(pattern_violation)  # Example: "Joel Embiid Violation:Lane"
re_violation_team = re.compile(
    pattern_violation_team
)  # Example: "WARRIORS Violation: Delay of game"

# Turnovers
re_turnover_player = re.compile(
    pattern_turnover_player
)  # Example: "Stephen Curry Bad Pass Turnover (P2.T5)"
re_turnover_team = re.compile(
    pattern_turnover_team
)  # Example: "WARRIORS Turnover: Shot Clock (T#12)"

# Game management
re_substitution = re.compile(
    pattern_substitution
)  # Example: "SUB: Andre Iguodala FOR Stephen Curry"
re_timeout = re.compile(
    pattern_timeout
)  # Example: "WARRIORS Timeout: Regular (Reg.4 Short 0)"
re_jump_ball = re.compile(
    pattern_jump_ball
)  # Example: "Jump Ball Embiid vs. Jokic: Tip to Harris"
re_ejection = re.compile(
    pattern_ejection
)  # Example: "Draymond Green Ejection: Second Technical"
...
eventmsgtype_to_re = defaultdict(
    list,
    {
        ...
        EventMsgType.FIELD_GOAL_MISSED: [
            ...
            re_block,
        ],  # Shot could be blocked
        EventMsgType.FREE_THROW: [
            re_free_throw_made,
            re_free_throw_miss,
        ],  # Both outcomes needed
