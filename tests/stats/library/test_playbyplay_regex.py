import pytest
from nba_api.stats.library.playbyplay_regex import *
...
class TestPlayByPlayRegex(object):
    ...
    def test_player_name(self):
        ...
        re_player_name = re.compile('^{player_name}'.format(player_name=patterns['player_name']))

        #Single Name
        player_name = "Millsap REBOUND (Off:0 Def:2)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == 'Millsap'

        #Hyphenated Name
        player_name = "Bates-Diop REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == 'Bates-Diop'

        #Name with Apostrophe
        player_name = "O'Quinn REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == "O'Quinn"

        #First Initial + Last Name
        player_name = "S. Hill REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == "S. Hill"

        #Junior
        player_name = "Porter Jr. REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == "Porter Jr."

        #Second
        player_name = "Payton II REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == "Payton II"

        #Third
        player_name = "Robinson III REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
        assert player_name == "Robinson III"

        #Fourth
        player_name = "Walker IV REBOUND (Off:0 Def:1)"
        player_name = re_player_name.search(player_name).group('player_name')
